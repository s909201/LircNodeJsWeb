...
// var sleep = require('sleep');  // modified by Morgan, 2019.11.05
...
router.post('/macro/:macro', function (req, res, next) {
    ...
    if (macro) {
        for (var i = 0, len = macro.length; i < len; i++) {
            ...
            execSync(command);
            if (step.delay) {
                // sleep.msleep(step.delay); // modified by Morgan, 2019.11.05
                setTimeout(function () { // modified by Morgan, 2019.11.05
                    callback(null); // modified by Morgan, 2019.11.05
                }, step.delay); // modified by Morgan, 2019.11.05
            }
        }
    } else {
